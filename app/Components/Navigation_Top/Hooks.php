...
class Hooks implements Bootable {
    ...
    public static function get_classes( $classes ) {
        ...
        if( app('nav/top/functions')->hide_the_nav() ) {
            $classes[] = 'hidden-when-not-mobile';
        }

        if( app('nav/top/functions')->hide_the_mobile_nav() ) {
            $classes[] = 'hidden-when-mobile';
        }

        if( app('nav/top/functions')->hide_the_fixed_nav() ) {
            $classes[] = 'hidden-when-not-fixed';
        }

        $classes[] = sprintf( 'mobile-at-%s', app('nav/top/functions')->get_breakpoint() );
